...
@function_tool
def get_meal_plan(user_id: int, plan_date: Optional[str] = None) -> Dict:
    """
    ...
    try:
        with get_db_context() as db:
            ...
            meal_plan = crud.meal_plan.get_by_date(
                db=db, user_id=validated_id, date=plan_date_obj
            )

        if not meal_plan:
            return format_success_response(
                message=f"No meal plan found for {plan_date_obj}", has_plan=False
            )
        ...
        return format_success_response(
            ...
            meal_plan={
                ...
                "updated_at": (
                    meal_plan.updated_at.isoformat() if meal_plan.updated_at else None
                ),
            },
        )
    except Exception as e:
