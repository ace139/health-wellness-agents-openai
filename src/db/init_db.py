...
def generate_sample_users(count: int = 100) -> List[schemas.UserCreate]:
    ...
    physical_limitations = [
        "None", "Mobility issues", "Visual impairment",
        "Hearing impairment", "Limited dexterity"
    ]
    ...
    for i in range(len(test_users), count):
        ...
        limitations = [
            limitation for limitation in limitations if limitation != 'None'
        ]
...
def generate_cgm_readings(
    ...
    user_conditions = {
        user.id: "diabetes" in (user.medical_conditions or "").lower()
        for user in users
    }
